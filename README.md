...
138 条断言，覆盖配对表（指纹规范化、去重、落盘、坏文件留底）、SAS 与滚动 rid 的
跨实现向量、配对码拼装、失败退避、配置读写，以及 §8.2 第 3 阶段那次明文迁移
（新装不提示、升级关掉并提示、只做一次、用户重新打开后必须留住）。其中的向量和 Android 端
...
> - 加密不可用 → 退回 `v=1`，里面是**明文 token**，等价于把本机的访问权交出去，
>   截图、转发、投屏都要当成泄露处理。
>
